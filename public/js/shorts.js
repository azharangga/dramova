...
(function () {
  ...
  const homePlatformBtn = document.getElementById('shortsPlatformBtn');
  const homePlatformLabel = document.getElementById('shortsPlatformLabel');

  let heroTimer = null;
  let heroIndex = 0;
  let heroSlides = [];
  const state = {
    platform: D.Store.get(D.STORAGE.SHORTS_PLATFORM, 'all') || 'all',
    page: 1,
    items: [],
    sectionItems: null,
    hasMore: true,
  ...
  function syncPlatformButton(platform = state.platform) {
    ...
    homePlatformLabel.textContent = platform === 'all'
      ? D.t('search.all_platforms')
      : (D.Platforms?.[platform]?.label || platform);
  }
  ...
  function pickHeroItems(items, limit = 5) {
    return shuffle((items || []).filter((item) => synopsisOf(item))).slice(0, limit);
  }
  ...
  async function enrichVisibleItems(items, platform) {
    ...
    // Item lain tampil apa adanya dari data home API
  ...
  // Smart sort: pakai recommendations kalau user punya history/favorites
  function smartSort(items) {
    if (D.sortByRecommendation) {
      const sorted = D.sortByRecommendation(items);
      // Kalau ada preferensi, pakai sorted. Kalau tidak, fallback ke stableShuffle
      if (sorted !== items) return sorted;
    }
  ...
  function homeSections(items) {
    ...
    const forYou = smartSort(remaining.filter((it) => !newReleaseIds.has(it.id)));
  ...
  function renderHomeSections(items, platform) {
    ...
    const forYouItems = sections.forYou.length
      ...
      : smartSort(items).slice(0, state.visibleForYou);
  ...
  function buildSectionItems(allItems) {
    ...
    return {
      ...
      forYou: forYou.length ? forYou : smartSort(allItems),
    };
  }

  async function loadHomeData(platform, page) {
    if (platform !== 'all') {
      const result = await loadPlatformPage(platform, page);
      return { ...result, sectionItems: null };
    }
    ...
    const items = [
      ...sectionItems.trending,
      ...sectionItems.newRelease,
      ...sectionItems.forYou,
    ];
    ...
    return {
      ...
      sectionItems,
      data: {
        hasMore: fulfilled.some((result) => result.value.data?.hasMore !== false && result.value.items.length >= 6),
      },
    };
  }
  ...
  if (homePlatformBtn) {
    homePlatformBtn.addEventListener('click', () => {
      D.openSheet({
        ...
        items: platformItems(),
        onPick: (id) => {
          const p = D.ALL_PLATFORMS.find((x) => x.id === id);
          if (p?.disabled) { D.toast?.info?.(`${p.label} sedang maintenance.`); return; }
          state.platform = id;
          D.Store.set(D.STORAGE.SHORTS_PLATFORM, id);
          if (id !== 'all') {
            D.setPlatform(id);
            return;
          }
